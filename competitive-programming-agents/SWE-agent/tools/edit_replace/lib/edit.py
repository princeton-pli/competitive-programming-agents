...
Please modify your search string. Did you forget to properly handle whitespace/indentation, or did you replace a line break with the literal `\n` character? You can also call `open` again to re-display the file with the correct context.
"""
...
1. The edited file is correctly indented and no line breaks are replaced with the literal `\n` character (recall that this is forbidden)
...
def edit(search: str, replace: str, replace_all: bool):
    try:
...
def insert(text: str, line: Union[int, None] = None):
    try:
        wf = WindowedFile(exit_on_exception=False)
    except FileNotOpened:
